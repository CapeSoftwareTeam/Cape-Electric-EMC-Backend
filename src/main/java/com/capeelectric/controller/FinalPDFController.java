...
public class FinalPDFController {
	...
	@GetMapping("/printFinalPDF/{userName}/{emcId}/{clientName}")
	@ResponseBody
	public ResponseEntity<byte[]> printFinalPDF(@PathVariable String userName, @PathVariable Integer emcId,@PathVariable String clientName) throws Exception {
		logger.info("called printFinalPDF function userName: {},emcId : {}", userName, emcId);

		ByteArrayOutputStream downloadInputStream = returnPDFService.printFinalPDF(userName, emcId,clientName);
